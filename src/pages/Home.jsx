...
const Home = () => {
  ...
  const { timeLeft } = useTimer(timerSeconds, roundKey, isPaused);
          ...
          <div className="mt-6 flex flex-wrap justify-center gap-2">
            {['easy', 'medium', 'hard'].map((level) => (
              <button
                ...
                className={`rounded-full border px-4 py-2 text-sm font-semibold capitalize transition ${difficulty === level
                  ? (isDarkMode ? 'border-cyan-400 bg-cyan-500/15 text-cyan-200' : 'border-cyan-500 bg-cyan-50 text-cyan-700')
                  : (isDarkMode ? 'border-slate-700 bg-slate-800/70 text-slate-300' : 'border-slate-300 bg-white text-slate-700')}`}
              >
                {level}
              </button>
            ))}
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2 sm:flex-row sm:items-center sm:gap-3">
              ...
              <div className={`rounded-full border px-3 py-1 text-sm ${isDarkMode ? 'border-cyan-500/30 bg-cyan-500/10 text-cyan-200' : 'border-cyan-400/30 bg-cyan-50 text-cyan-700'}`}>
                {difficultyLabel} · Round {questionIndex + 1}
              </div>
              <button
                onClick={() => {
                  setStarted(false);
